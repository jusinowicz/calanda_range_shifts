...
f.name1=c("calanda_ccs26_tsmfull4_2017")
...
#hnk = cbind( matrix(3,5,1), matrix(4,5,1);
...
n_runs = 1000 #Draws from PD of flowering for LGR prediction intervals
...
###
###Dispersal distance. Not implemented here. These are calculated using 
###the WALD model --see code below and wald_model1.R
###
...
Vt=c(2.6,2.91,3.81) #terminal velocity: See LEDA database
#Model parameters for the Beta function: Based on Su et al. 2001
a1=1.05
a2=2
a3=0.1

#Choose this for just the average height!
mean_only = TRUE 
#mean_only = FALSE

if(mean_only == TRUE) { 
	kd=matrix(0,np,nspp) #array(c(matrix(0,np,1),matrix(0,np,1)),dim=c(np,1,nspp) )
}else{
...
for( s in 1:nspp){ 
	...
	if(mean_only == TRUE) { 
		#Mean height -- much faster, because mvfft can be slow. 
		wald.list[[s]] = get.WALD.kernel(u_mean, xx0, height_k2[,s]/100, Vt[s],a1,a2,a3 )[[1]][[1]]
		wald.list[[s]] = as.matrix(wald.list[[s]][,1])#Just pick 1 and reduce matrix size for speed. 
		...
		kd[,s]=kd[,s]/matrix(sum(kd[,s],na.rm=T),np,1,byrow=T)
		kd[,s][is.na(kd[,s])] = 0
...
for( t in 1: ngenst){ 
	...
	for(sp in 1:length(spp)){
		...
		#q1=quantile(mean.model,c(.025,.975))
		rr_ci_tmp = t(apply(model_post, 1, quantile, c(0.05,0.95)))
	...
	for( sa in 1:nspp){
		...
		Frs_q1 = apply(Frs_post[[t]][[sa]], 1, quantile, c(0.05,0.95))
		Frs_ci[[t]][[sa]] = Frs_q1
