...
#For naming files
#=============================================================================
f.name1=c("calanda_igr5_multi_2017")
#=============================================================================
...
#Temp
temp_gam = gam( gs_mean_temp~ year+s(elevation,k=5),data=allB)
#temp_gam = gam( gs_mean_temp~ year+elevation+e2,data=allB)

#soil moisture 
sm_gam = gam( soil_moist~ year+s(elevation,k=5),data=allB)
#sm_gam = gam( soil_moist~ year+elevation+e2,data=allB)

#PET
sp_gam = gam( soil_pet~ year+s(elevation,k=5),data=allB)
#sp_gam = gam( soil_pet~ year+elevation+e2,data=allB)

#Moisture deficit
sd_gam = gam( soil_def~ year+s(elevation,k=5),data=allB)
#sd_gam = gam( soil_def~ year+elevation+e2,data=allB)

#annual mean light
ml_gam = gam( an_mean_light~ year+s(elevation,k=5),data=allB)
#ml_gam = gam( an_mean_light~ year+elevation+e2,data=allB)

#GDD
gdd_gam = gam( max_gdd~ year+s(elevation,k=5),data=allB)
#gdd_gam = gam( max_gdd~ year+elevation+e2,data=allB)

#Minimum growing season temp
gmt_gam = gam( gs_min_temp~ year+s(elevation,k=5),data=allB)
#gmt_gam = gam( gs_min_temp~ year+elevation+e2,data=allB)
...
#=============================================================================
# Future environment: Use the model to predict values: 
xx_new = xx
minc = 1.5 
xx_new$gs_mean_temp = as.matrix(xx_new$gs_mean_temp+minc)
...
for(sp in 1:length(spp)){
	...
	#1)
	flower_ptmp= gam(flyes ~ s(year,bs="re")+s(elevation,k=5)+s(gs_mean_temp,k=5)+s(soil_def,k=5)+s(max_gdd,k=5)+s(gs_min_temp),family=binomial(link='logit'),data=rr_dat)

	#2)
	#flower_ptmp= gam(flyes ~ s(year,bs="re")+s(gs_mean_temp,k=5)+s(soil_def,k=5)+s(max_gdd,k=5)+s(gs_min_temp),family=binomial(link='logit'),data=rr_dat)
	#flower_ptmp= gam(flyes ~ s(year,bs="re")+s(gs_mean_temp,k=5)+s(soil_def,k=5)+s(max_gdd,k=5)+s(gs_min_temp),family=binomial(link='logit'),data=rr_dat)


	#3)
	#flower_ptmp= gam(flyes ~ s(year,bs="re")+s(gs_mean_temp,k=5)+s(soil_def,k=5)+s(gs_min_temp,k=5),family=binomial(link='logit'),data=rr_dat)
	#flower_ptmp= gam(flyes ~ s(year,bs="re")+s(gs_mean_temp,k=5)+s(soil_def,k=5)+s(gs_min_temp,k=5)
	...
	#4)
	#flower_ptmp= gam(flyes ~ s(year,bs="re")+s(gs_mean_temp,k=5)+s(elevation,k=5)+s(gs_min_temp),family=binomial(link='logit'),data=rr_dat)

	#5)
	#flower_ptmp= gam(flyes ~ s(year,bs="re")+s(elevation,k=5)+s(gs_mean_temp,k=5)+s(gs_min_temp,k=5)+te(elevation, gs_mean_temp,gs_min_temp),family=binomial(link='logit'),data=rr_dat)

	#6)
	#flower_ptmp= gam(flyes ~ s(year,bs="re")+s(elevation,k=5)+s(gs_mean_temp,k=5)+te(elevation, gs_mean_temp),family=binomial(link='logit'),data=rr_dat)


	# flower_ptmp= gam(flyes ~ s(year,bs="re")+s(gs_mean_temp,k=5)+s(soil_def,k=5)
	# 	+s(an_mean_light,k=5)+ s(an_min_temp,k=5)+s(max_gdd),family=binomial(link='logit'),data=rr_dat)

	#flower_ptmp= gam(flyes ~ s(year,bs="re")+s(gs_mean_temp,k=5)+s(soil_def,k=5)
	#	+ s(an_min_temp,k=5)+s(max_gdd),family=binomial(link='logit'),data=rr_dat)

	#flower_ptmp= gam(flyes ~ s(year,bs="re")+s(gs_mean_temp,k=5)+s(soil_def,k=5)+te(gs_mean_temp,soil_def),family=binomial(link='logit'),data=rr_dat)
	#flower_ptmp= gam(flyes ~ s(year,bs="re")+s(soil_moist,k=5)+s(gs_mean_temp,k=5)+s(soil_def,k=5)+te(soil_moist, gs_mean_temp,soil_def),family=binomial(link='logit'),data=rr_dat)

	#flower_ptmp= gam(flyes ~ s(year,bs="re")+s(elevation,k=5)+s(gs_mean_temp,k=5)+s(soil_def,k=5)+te(elevation, gs_mean_temp,soil_def),family=binomial(link='logit'),data=rr_dat)
	#flower_ptmp= gam(flyes ~ year+elevation+e2+s(gs_mean_temp,k=5)+s(soil_def,k=5),family=binomial(link='logit'),data=rr_dat)
	#flower_ptmp= gam(flyes ~ year+elevation+e2+gs_mean_temp+gs2+s(soil_def,k=5),family=binomial(link='logit'),data=rr_dat)
...
for(sp in 1:length(spp)){
	...
	#1)
	rr_gam= gam(nr.inflor ~ s(year,bs="re")+s(elevation,k=5)+s(gs_mean_temp,k=5)+s(soil_def,k=5)+s(max_gdd,k=5)+s(gs_min_temp),data=rr_dat)

	#2)
	#rr_gam= gam(nr.inflor ~ s(year,bs="re")+s(gs_mean_temp,k=5)+s(soil_def,k=5)+s(max_gdd,k=5)+s(gs_min_temp),data=rr_dat)

	#3)
	#rr_gam= gam(nr.inflor ~ s(year,bs="re")+s(gs_mean_temp,k=5)+s(soil_def,k=5)+s(gs_min_temp),data=rr_dat)
	...
	#rr_gam= gam(nr.inflor ~ year+s(elevation,k=5)+s(gs_mean_temp,k=5)+s(soil_def,k=5)+te(elevation, gs_mean_temp,soil_def),data=rr_dat)


	#Use polynomial model (assumes elevation^2 exists)
	#rr_gam = gam(nr.inflor~s(year,bs="re")+elevation+e2, data=rr_dat)
	#rr_gam = gam(nr.inflor~s(year,bs="re")+s(elevation,k=5)+s(gs_mean_temp,k=5)+s(soil_def,k=5), data=rr_dat)
	#rr_gam = gam(nr.inflor~s(year,bs="re")+elevation+e2+gs_mean_temp+gs2+soil_def+sd2, data=rr_dat)
	#rr_gam = gam(nr.inflor~s(year,bs="re")+elevation+e2+gs_mean_temp+gs2, data=rr_dat)

	#rr_gam= gam(nr.inflor ~ s(year,bs="re")+s(gs_mean_temp,k=5)+s(soil_def,k=5)
	#	+s(an_mean_light,k=5)+ s(an_min_temp,k=5)+s(max_gdd),data=rr_dat)
...
####Intrinsic ranges
#FrsS=array(c(rr_krigS*flower_probKS),dim=c(ngenst,np,nspp)) 
#FrsS[,,1] = FrsS[,,1]*4
...
#Simulation IGR
ldg.sim.p = matrix(0,ngenst, nspp*(nspp))
ldgIMP.sim.p = matrix(0,ngenst, nspp*(nspp))
covIMP.sim.p= matrix(0,ngenst, nspp*(nspp))

#Components of IGR
l1.p=matrix(0,ngenst,nspp*(nspp))
y.p=matrix(0,ngenst,nspp*(nspp))
w.eq.p=matrix(0,ngenst,nspp*(nspp))
D.p = matrix(0,ngenst,nspp*(nspp))
var_mu_Us.p=matrix(0,ngenst,nspp*(nspp)) 
cov_e_mu_Us.p=matrix(0,ngenst,nspp*(nspp))
cov_lam_vc.p=matrix(0,ngenst,nspp*(nspp))
cov_lam_vc.p2=matrix(0,ngenst,nspp*(nspp))
Elam1.p=matrix(0,ngenst,nspp*(nspp)) 
Elam2.p=matrix(0,ngenst,nspp*(nspp))
gr1.n.p=matrix(0,ngenst,nspp*(nspp))
gr1.p=matrix(0,ngenst,nspp*(nspp))
...
for ( s in 1:nspp) { 
	...
	for( t in 1: ngenst){ 
		...
		for (sp in 1:(nspp)){ 
			...
			nr.p[t,,s.index1[sp]] = get.res.eq(Frs[t,,],s.index1[sp],sr,alphas, fkd,kc,fkd.yes,fast=TRUE, burns = 5)
		}
		...
		# 	}
			...
			for (sp in 1:(nspp)){ 
				if(s<s.index1[sp]) {s.inv=1}else{s.inv=2}
				#Get the pairwise invader low-density equilibrium 
				ni.p[t,,s.index1[sp]] =get.inv.ldeq(Frs[t,,sort(c(s,s.index1[sp]))], nr.p[t,,sort(c(s,s.index1[sp]))], s.inv, sr,alphas, fkd,kc,fkd.yes,fast=TRUE, burns = 5 )
			}
	...
	#In this case, also get the invader against itself
	...
	for (rs in 1:(nspp)) {
		...
		for (t in 1:ngenst) { 
			...
			y.p[t,enter1]=mean(nr.p[t,,rez.no])#Means of resident densities 
			...
			nrs.p=Re(fft((alphas[s,rez.no]*fft(nr.p[t,,rez.no])*fkc[,rez.no]),inverse=T)/(np+1))
			nrs.p=c(nrs.p[ceiling(np/2):(np)],nrs.p[1:floor(np/2)])
		
			#Calculate the standardized competition from residents
			#muj.p = nr2_eq_all[t,,rez.no]/y.p[t,enter1]-1
			muj.p = nr.p[t,,rez.no]/y.p[t,enter1]-1
