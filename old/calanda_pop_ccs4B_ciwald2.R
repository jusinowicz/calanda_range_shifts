...
#For naming files
#=============================================================================
f.name1=c("calanda_ccs26B_tsmfull_2017")
#=============================================================================
...
#=============================================================================
#Somehow gs_mean_temp got messed up for allB2. Fix this later. For now:
...
xx=data.frame( elevation=xx1, year=yx, e2=xx2)
...
for( t in 1: ngenst){ 
	...
	flower_prob_act = matrix(0,length(elevations),3)
	...
	#Use this piece of code to set upper and lower range limits
	#These define the spatial extent of the study area
	r_lims = TRUE
	#pad_where = c(500, 800,2200, 2500)
	pad_temp = c(min(allB2$gs_mean_temp,na.rm=T)-2,max(allB2$gs_mean_temp,na.rm=T)+2)
	closest_pad = c( which.min(abs(xx$gs_mean_temp-pad_temp[1])),which.min(abs(xx$gs_mean_temp-pad_temp[2]))) 
	#pad_where = c(800,2200)
	pad_where = c(xx$elevation[closest_pad[2]], xx$elevation[closest_pad[1]])
	pad_val = 0
	pad_xx = xx[ 1: (length(years)*length(pad_where)), ]

	for (ay in 1: length(years)){ 
		for(ab in 1:length(pad_where)){
			
			pad_xx[ (length(pad_where)*(ay-1)+ab),] = (xx[which(xx$elevation == pad_where[ab]),])
			pad_xx$year[(length(pad_where)*(ay-1)+ab) ]=years[ay]
		}
	}
	...
	for(sp in 1:length(spp)){
		...
		e_fix = unique(rr_dat$elevation[is.na(rr_dat$gs_mean_temp)]	)
		for (ef in 1:length(e_fix)) { 
			rr_dat$gs_mean_temp[rr_dat$elevation == e_fix[ef] ] = 
				xx[which(xx$elevation == e_fix[ef]),]$gs_mean_temp
		}

		#Create range limits by padding real data with fake data in 
		#the form of zeros to indicate that species do not reproduce beyond
		#a certain region. 
		if(r_lims == TRUE) { 
			n_sampl = dim(rr_dat)[1]/(5*3)
			pad_xx_add = pad_xx[rep(1:nrow(pad_xx),n_sampl),]
			rr_dat=rbind.fill(rr_dat,as.data.frame( pad_xx_add))
			rr_dat[is.na(rr_dat)] = 0
		}
		


		#What are actual per-site probs? 
		for (n in 1:length(elevations)) { 
			tfp = subset(rr_dat, elevation == elevations[n])
			#flower_prob_act[n,sp]= sum(tfp$flyes)/nrow(tfp)}
			flower_prob_act[n,sp]= sum(tfp$flyes)} #sum(tfp$flyes)/nrow(tfp)}
		...
		if( sp == 1) { 
			...
			#flower_ptmp= gamm4(flyes ~ t2(gs_mean_temp,soil_moist,gs_min_temp),random = ~(1|year),family=binomial(link='logit'),data=rr_dat)
			flower_ptmp= gamm4(flyes ~ s(gs_mean_temp,k=kn[sp])+s(soil_moist,k=kn[sp])+s(gs_min_temp,k=kn[sp]),random = ~(1|year),family=binomial(link='logit'),data=rr_dat)
			...
			flower_ptmp= gamm4(flyes ~ s(gs_mean_temp,k=kn[sp])+s(max_gdd,k=kn[sp])+s(gs_min_temp,k=kn[sp])+s(soil_moist,k=kn[sp]),random = ~(1|year),family=binomial(link='logit'),data=rr_dat)
			...
			flower_ptmp= gamm4(flyes ~ s(gs_mean_temp,k=kn[sp])+s(gs_min_temp,k=kn[sp])+s(soil_moist,k=kn[sp]),random = ~(1|year),family=binomial(link='logit'),data=rr_dat)
		...
		#flower_ptmp= gamm4(flyes ~ s(gs_mean_temp, k=kn[sp]),random = ~(1|year),family=binomial(link='logit'),data=rr_dat)
		#flower_ptmp= gamm4(flyes ~ s(gs_mean_temp,k=kn[sp])+s(soil_moist,k=kn[sp]),random = ~(1|year),family=binomial(link='logit'),data=rr_dat)
		...
		#This correction to the PIs is necessary for sp 2. Not sure why
		if(sp == 2) {
			lfp = Xp%*%t(br1)
			md1 = max(log(flower_tmp))
			md2 = max(rowMeans(lfp))
			m_diff =md2-md1
			flower_prob_post[[sp]] = exp(lfp - m_diff ) 
			flower_probK[,sp]= rowMeans(flower_prob_post[[sp]] )
		}
	...
	kn = c(3,3,3)
	flower_act =matrix(0,length(elevations),3)
	...
	for(sp in 1:length(spp)){
		...
		#rr_gam= gamm4(nr.inflor ~ s(gs_mean_temp, k=kn[sp]),random = ~(1|year),data=rr_dat)
		rr_gam= gamm4(nr.inflor ~ s(gs_min_temp,k=kn[sp])+s(gs_mean_temp,k=kn[sp])+s(soil_moist,k=kn[sp]),random = ~(1|year),data=rr_dat)


		#Use the model to predict values: 
		#Under warmer conditions, uncomment:
		rr_tmp=as.vector(predict.gam(rr_gam$gam, newdata=xx_new,type= "response"))
