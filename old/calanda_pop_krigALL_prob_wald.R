...
xx= as.matrix(seq(900,2100,1))
yx= matrix(2016,length(xx),1)
xx=cbind(xx,yx)
colnames(xx)=c('elevation','year')
xx=data.frame(xx)
...
#=============================================================================
#Calculate constant annual survival rate for each species, independent of treatments 

surv_spp =matrix(0,length(spp),1)
 for(sp in 1:length(spp)){
 	allsp = subset(allB, Sp == spp[sp])
 	yr_surv = matrix(1,dim(allsp)[1]/length(years), length(years)+1 )
 		
 		for(yr in 1:length(years)){ 
 				yr_surv[,(yr+1)] = as.numeric((subset(allsp, year == years[yr]))$survival)-1
 			}
 	#proportion surviving
 	s1=colSums(yr_surv)/colSums(yr_surv)[1]
 	#survival rate: 1 -mortality rate = 1 - (proportion dying)/proportion surviving 
 	sr=1-diff(1-s1)/s1[1:3]
 	surv_spp[sp] = mean(sr)
}

#variable kriged survival -- area does not do well as a predictor
sr_krig = matrix(0,dim(xx)[1], length(spp))
sr_prob=NULL
kn = c(4,4,4)
for(sp in 1:length(spp)){
	sr_dat = subset(allB, Sp == spp[sp])
...
#Look for competitive effects of survival 
#variable kriged survival -- competition vs. bare
sr_krigC1 = matrix(0,dim(xx)[1], length(spp))
sr_probC1=NULL
kn = c(4,4,4)
for(sp in 1:length(spp)){
	#allsp = subset(allB, Sp == spp[sp])
	#sr_dat=subset(allsp, bg =='B')
	sr_dat = subset(allC1, Sp == spp[sp])
	sr_gam = gam(survival~C1+s(elevation,k=kn[sp])+s(year,bs="re"),family=binomial(link='logit'), data=sr_dat)
	#sr_tmp=as.vector(predict.gam(sr_gam, newdata=xx,type= "response"))
	#Remove 0s
	sr_tmp[sr_tmp<0] = 0
	sr_krigC1[,sp] = sr_tmp
...
#variable kriged survival -- inter, intra, and bare
sr_krigChc = matrix(0,dim(xx)[1], length(spp))
sr_probChc=NULL
kn = c(4,4,4)
for(sp in 1:length(spp)){
	#allsp = subset(allB, Sp == spp[sp])
	#sr_dat=subset(allsp, bg =='B')
	sr_dat = subset(allChc, Sp == spp[sp])
	sr_gam = gam(survival~Chc+s(elevation,k=kn[sp])+s(year,bs="re"),family=binomial(link='logit'), data=sr_dat)
	#sr_tmp=as.vector(predict.gam(sr_gam, newdata=xx,type= "response"))
	#Remove 0s
	sr_tmp[sr_tmp<0] = 0
	sr_krigChc[,sp] = sr_tmp
	sr_probChc[[sp]] = sr_gam
}

#variable kriged survival -- species-specific and bare
sr_krigC = matrix(0,dim(xx)[1], length(spp))
sr_probC=NULL
kn = c(4,4,4)
for(sp in 1:length(spp)){
	#allsp = subset(allB, Sp == spp[sp])
...
#AICs
AIC(sr_probB[[1]],sr_prob[[1]],sr_probC1[[1]],sr_probChc[[1]],sr_probC[[1]])
AIC(sr_probB[[2]],sr_prob[[2]],sr_probC1[[2]],sr_probChc[[2]],sr_probC[[2]])
AIC(sr_probB[[3]],sr_prob[[3]],sr_probC1[[3]],sr_probChc[[3]],sr_probC[[3]])


#The pattern is similar for all three species: the model using only Bare gives
#the best AIC. But then: 
# DG: sr_probChc[[1]] 7.747963 594.41678
# AX: sr_probChc[[2]] 7.518139 568.84109
# HN: sr_probC[[3]]   8.623584 886.7293

#Plot kriged values
par(mfrow=c(3,1))
for(sp in 1:3) {
	plot(sr_krig[,sp],ylim=c(0,1))
	points(sr_krigB[,sp],col="red")
}
...
#=============================================================================
#=============================================================================
#Calculate average intraspecific invasion growth rate (flowers) per site
lrr_krig = matrix(0,dim(xx)[1], length(spp))
kn = c(4,4,4)
for(sp in 1:length(spp)){
	allsp = subset(allB, Sp == spp[sp])
	lrr_dat =subset(allsp, bg == substr(as.character(spp[sp]),1,1))
	lrr_gam = gam(nr.inflor~s(elevation,k=kn[sp])+s(year,bs="re"), data=lrr_dat)
	print(summary(lrr_gam))
	lrr_tmp=as.vector(predict.gam(lrr_gam, newdata=xx))
	#Remove 0s
	lrr_tmp[lrr_tmp<0] = 0
	lrr_krig[,sp] = lrr_tmp
}
#=============================================================================
#Krig the average intraspecific competition (flowers) per site
#This version matches individuals within a plot -- not very effective, given
#This reflects individual (within-plot) level variation. Is this useful in this
#project? 

Crr_krig = matrix(0,dim(xx)[1], length(spp))
kn = c(4,4,4)

for(sp in 1:length(spp)){
	allsp = subset(allB, Sp == spp[sp])
	Crr_dat =subset(allsp, bg == substr(as.character(spp[sp]),1,1))
	rr_sub =subset(allsp, bg =='B')
	Crr_dat$intra_flor = 1/(Crr_dat$nr.inflor/rr_sub$nr.inflor)   
	Crr_dat$intra_flor[is.infinite(Crr_dat$intra_flor)] = 0
	Crr_gam = gam(intra_flor~s(elevation,k=kn[sp])+s(year,bs="re"), data=Crr_dat)
	print(summary(Crr_gam))
	Crr_tmp=as.vector(predict.gam(Crr_gam, newdata=xx))
	#Remove 0s
	Crr_tmp[Crr_tmp<0] = 0
	Crr_krig[,sp] = Crr_tmp
}
...
for(sp in 1:length(spp)){
	...
	rr_sub =subset(allsp, bg =='B')
	crdat= data.frame(cbind(Cr_dat$nr.inflor, rr_sub$nr.inflor))
	...
	for(ee in 1:length(elevations)){
		...
		crdat_e= data.frame(cbind(Cr_dat_e$nr.inflor, rr_sub_e$nr.inflor))
...
#=============================================================================
#=============================================================================
#Calculate average interspecific invasion growth rate (flowers) per site
lii_all = NULL
kn = matrix(4,length(spp),length(spp))

for(sp in 1:length(spp)){
	lii_krig = matrix(0,dim(xx)[1], length(spp)-1)
	allsp = subset(allB, Sp == spp[sp])
	sp_use = 1:(length(spp))
	sp_use= sp_use[-sp]
	...
	for(sa in 1: (length(spp)-1)){
		...
		sb= sp_use[sa]
		lii_dat =subset(allsp, bg == substr(as.character(spp[sb]),1,1))
		lii_gam = gam(nr.inflor~s(elevation,k=kn[sp,sb])+s(year,bs="re"), data=lii_dat)
		print(summary(lii_gam))
		lii_tmp=as.vector(predict.gam(lii_gam, newdata=xx))
		#Remove 0s
		lii_tmp[lii_tmp<0] = 0
		lii_krig[,sa] = lii_tmp
	}
	lii_all[[sp]] = list(lii_krig)
}

#=============================================================================
#Krig the average interspecific competition (flowers) per site
#This version matches individuals within a plot -- not very effective, given
#This reflects individual (within-plot) level variation. Is this useful in this
#project?
...
for(sp in 1:length(spp)){
	Cir_krig = matrix(0,dim(xx)[1], length(spp)-1)
	allsp = subset(allB, Sp == spp[sp])
	rr_sub =subset(allsp, bg =='B')
	sp_use = 1:(length(spp))
	sp_use= sp_use[-sp]
	...
	for(sa in 1: (length(spp)-1)){
		...
		sb= sp_use[sa]
		Cir_dat =subset(allsp, bg == substr(as.character(spp[sb]),1,1))
		Cir_dat$inter_flor = 1/(Cir_dat$nr.inflor/rr_sub$nr.inflor)   
		Cir_dat$inter_flor[is.infinite(Cir_dat$inter_flor)] = 0
		Cir_gam = gam(inter_flor~s(elevation,k=kn[sp,sb])+s(year,bs="re"), data=Cir_dat)
		print(summary(Cir_gam))
		Cir_tmp=as.vector(predict.gam(Cir_gam, newdata=xx))
		#Remove 0s
		Cir_tmp[lii_tmp<0] = 0
		Cir_krig[,sa] = Cir_tmp
	}
	Cir_all[[sp]] = list(Cir_krig)
}
...
for(sp in 1:length(spp)){
	...
	for(sa in 1: (length(spp)-1)){
		...
		cidat= data.frame(cbind(Ci_dat$nr.inflor, rr_sub$nr.inflor))
		colnames(cidat) = c("lir","Ri")
		Ci_nls = nls(lir~ Ri/(1+ci),data=cidat, start=list(ci=0.5))
		...
		for(ee in 1:length(elevations)){
			...
			rr_sub_e = subset(rr_sub, elevation == elevations[ee])
			cidat_e= data.frame(cbind(Ci_dat_e$nr.inflor, rr_sub_e$nr.inflor))
			colnames(cidat_e) = c("lir","Ri")
			points(cidat_e$Ri ,cidat_e$lir )
			tryCatch( {Ci_nls_e = nls(lir~ Ri/(1+ci),data=cidat_e, start=list(ci=0.5),control = list(maxiter = 500))
...
#Version 1: use the kriged rrs

arat = matrix(0,length(spp),length(spp))
arat_all = NULL

for(sp in 1:length(spp)){
	a1 = matrix(0,dim(xx)[1], length(spp)-1)
	sp_use = 1:(length(spp))
	sp_use= sp_use[-sp]
	...
	for(sa in 1: (length(spp)-1)){
		sb= sp_use[sa]
		#Use medians of IGRs
		sub1 = (lrr_krig[,sb])/(lii_all[[sp]][[1]][,sa])
		sub2 = (lii_all[[sp]][[1]][,sa]+rr_krig[,sp])/(lrr_krig[,sa] + rr_krig[,sb])
		a1[,sa]=sub1*sub2
...
#For flowers, this doesn't include the survival term so: 
#air/arr = (m*(n+li))/((m+lr)*n)
#Version 2: use the kriged Cs

arat2 = matrix(0,length(spp),length(spp))
arat_all2 = NULL

for(sp in 1:length(spp)){
	c1 = matrix(0,dim(xx)[1], length(spp)-1)
	sp_use = 1:(length(spp))
	sp_use= sp_use[-sp]
	...
	for(sa in 1: (length(spp)-1)){
		sb= sp_use[sa]
		#Use medians of IGRs
		c1[,sa]=(Cir_all[[sp]][[1]][,sa]-1)/(Crr_krig[,sa]-1)
		
	}
	arat_all2[[sp]] = list(c1)
}		
...
for ( s in 1:nspp) { 
	...
	for (t in 1:ngenst) { 
		inv.one =pop_lg(Frs[t,,],nr[t,,], sr, alphas, fkd, kd.yes, kc )[,s]
	...
	for (rs in 1:(nspp-1)){
		...
		for (t in 1:ngenst) { 
			...
			inv.one.p =pop_lg(Frs[t,,c(s,rez.no)],nr.inv,sr, alphas, kd,kc )[,1]
...
file.name = (paste(f.name1, ".var",sep=""))
