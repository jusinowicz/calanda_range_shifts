...
#For naming files
#=============================================================================
f.name1=c("calanda_ccs26_tsmfull_2017")
#=============================================================================
...
for( t in 1: ngenst){ 
	...
	for(sp in 1:length(spp)){
		...
		for (n in 1:length(elevations)) { 
			...
			#flower_ptmp= gam(flyes ~ year+te(gs_mean_temp,soil_def,gs_min_temp),family=binomial(link='logit'),data=rr_dat)
	...
	for(sp in 1:length(spp)){
		...
		rr_gam= gamm4(nr.inflor ~ s(gs_mean_temp,k=kn[sp])+s(soil_moist,k=kn[sp]),random = ~(1|year),data=rr_dat)
