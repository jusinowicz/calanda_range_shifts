...
#For naming files
#=============================================================================
f.name1=c("calanda_igr9_multiC_2017")
#=============================================================================
...
for(sp in 1:length(spp)){
	...
	if( sp == 1) { 
		...
		flower_ptmp= gam(flyes ~ s(year,bs="re")+s(elevation,k=kn[sp])+s(gs_mean_temp,k=kn[sp])+s(soil_def,k=kn[sp])+s(max_gdd,k=kn[sp])+s(gs_min_temp,k=kn[sp]),family=binomial(link='logit'),data=rr_dat)

	} else if (sp == 3) {

		flower_ptmp= gam(flyes ~ s(year,bs="re")+s(gs_mean_temp,k=kn[sp])+s(soil_def,k=kn[sp])+s(gs_min_temp,k=kn[sp]),family=binomial(link='logit'),data=rr_dat)
	}
...
for(sp in 1:length(spp)){
	...
	#1)
	rr_gam= gam(nr.inflor ~ s(year,bs="re")+s(elevation,k=kn[sp])+s(gs_mean_temp,k=kn[sp])+s(soil_def,k=kn[sp])+s(gs_min_temp,k=kn[sp]),data=rr_dat)
