...
#For naming files
#=============================================================================
f.name1=c("calanda_igr12_multi_2017")
#=============================================================================
...
lded.p = array(c(matrix(0.0,ngenst,np),matrix(0.0,ngenst,np)),dim=c(ngenst,np,nspp*nspp)) 
...
for ( s in 1:nspp) { 
	...
	for( t in 1: ngenst){ 
		...
		for (sp in 1:(nspp)){ 
			...
			nr.p[t,,s.index1[sp]] = get.res.eq(Frs[t,,],s.index1[sp],sr,alphas, fkd,fkc,fkd.yes,fast=TRUE, burns = 5)
		}
		...
		# 	}
			...
			for (sp in 1:(nspp)){ 
				...
				ni.p[t,,s.index1[sp]] =get.inv.ldeq(Frs[t,,sort(c(s,s.index1[sp]))], nr.p[t,,sort(c(s,s.index1[sp]))], s.inv, sr,alphas, fkd,fkc,fkd.yes,fast=TRUE, burns = 5 )
				

			}

			lded.p[t,,(1+(s-1)*nspp):(nspp*s)]
	...
	for (rs in 1:(nspp)) {
		...
		for (t in 1:ngenst) { 
			...
			inv.one.p =pop_lg(Frs[t,,c(s,rez.no)],nr.inv,sr, alphas,fkd, fkc, fkd.yes)[,1]
...
"Elam1.p","Elam2.p", "gr1.n.p", "gr1.p","ldg.sim","ldgIMP.sim","covIMP.sim","ldg.sim.p",
"lded","lded.p", "ldgIMP.sim.p","covIMP.sim.p","Frs")
