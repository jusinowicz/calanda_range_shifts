...
"cov_lam_vc.all", "cov_lam_vc2.all", "Elam1.all", "Elam2.all", "gr1.n.all", "gr1.all", "y.full.all", 
"w.eq.full.all","ldg.sim.all", "ldgIMP.sim.all", "covIMP.sim.all", "Frs.all", "env_analogue_all",
"sim.impacts.all", "sim.impactsIMP","covIMP.impacts","lded")


#List of scenario file names 
#file.name.list= list("calanda_repeat2_multi_2017_waldmean.var")
file.name.list= list("calanda_repeat3_multi_2017_waldmean.var")
...
ns=dim(sc1Frs.all[[1]])[2] #Lattice width/height -- This should be even
...
fig.name = paste("seeds_kriged_multi_igrsites3_2.pdf",sep="")
...
# fig.name = paste("calanda_widthOverlap_repeat_igr3b.pdf",sep="")
# pdf(file=fig.name, height=7.5, width=7.5, onefile=TRUE, family='Helvetica', pointsize=16)
...
for( g in 1:1){ 
	...
	x.axx= round(matrix( c(xlims[,1],xlims[,2],(( xlims[,2]-xlims[,1]) / 3) ),ngenst,3),1)
	y.axx= round(matrix( c(ylims[,1],ylims[,2],(( ylims[,2]-ylims[,1]) / 3) ),ngenst,3),1)
...
ldg.sim.a=t(matrix(unlist(sc1ldg.sim.all[1:7]),3,7))
...
lmm1=lm(ldg.sim.a[,1]~c1)
lmm2=lm(ldg.sim.a[,2]~c1)
lmm3=lm(ldg.sim.a[,3]~c1)

fig.name = paste("calanda_noveltylm_ldg3.pdf",sep="")
pdf(file=fig.name, height=7.5, width=7.5, onefile=TRUE, family='Helvetica', pointsize=16)

plot(c1, ldg.sim.a[,1],ylim=c(0,15),ylab="LDG",xlab ="average novelty")
points(c1, ldg.sim.a[,2],col="red")
points(c1, ldg.sim.a[,3],col="blue")
