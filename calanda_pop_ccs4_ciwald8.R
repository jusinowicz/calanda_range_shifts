...
for( t in 1: ngenst){ 
	...
	for(sp in 1:length(spp)){
		...
		peak_fp = el_real[flower_prob_act[,sp]==max(flower_prob_act[,sp])]
		fp_mat = subset(allsp, elevation == peak_fp)
		peak_tmp_f = mean(fp_mat$gs_mean_temp)
		...
		flower_tmp = Xp%*%t(beta) #These are not on the response scale
	...
	for(sp in 1:length(spp)){
		...
		### Find the elevation at which flowering peaks, then get the 
		### average value of variables corresponding to the peak elevation. 
		peak_fa = el_real[flower_act[,sp]==max(flower_act[,sp],na.rm=T)]
		fa_mat = subset(allsp, elevation == peak_fa)
		peak_tmp = mean(fa_mat$gs_mean_temp)
